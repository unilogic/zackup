/* $Id: flotr.js 144 2009-08-16 17:18:55Z fabien.menager $ */
...
// To fix a bug for Flotr.Color.toString()
Class.Mutators.toString = Class.Mutators.valueOf = function(method){
	return method;
};

// A few extensions to Mootools
Array.implement({
   ...
   */
	...
	getSeries: function(data){
		return data.map(function(serie){
			var i;
			serie = (serie.data) ? $extend({}, serie) : {data: serie};
	...
	merge: function(src, dest){
		...
		for(i in src){
      v = src[i];
			result[i] = (v && typeof(v) === 'object' && !(v.constructor === Array || v.constructor === RegExp) && $type(v) !== 'element') ? Flotr.merge(v, dest[i]) : result[i] = v;
		}
		return result;
	},
	/**
	...
	clone: function(object){
		...
		for(i in object){
			v = object[i];
			clone[i] = (v && typeof(v) === 'object' && !(v.constructor === Array || v.constructor === RegExp) && $type(v) !== 'element') ? Flotr.clone(v) : v;
		}
		return clone;
	},
	/**
...
Flotr.Graph = new Class({
	...
	initialize: function(el, data, options){
		...
		this.el.fireEvent('flotr:beforeinit', [this]);
		...
		for (type in Flotr.graphTypes) {
			this[type] = $extend({}, Flotr.graphTypes[type]);
			for (p in this[type]) {
				if ($type(this[type][p]) === 'function')
		...
		this.el.fireEvent('flotr:afterconstruct', [this]);
		...
		this.el.fireEvent('flotr:afterinit', [this]);
	},
	/**
	...
	setOptions: function(opts){
		...
		options.x2axis = $extend($extend({}, options.xaxis), options.x2axis);
		options.y2axis = $extend($extend({}, options.yaxis), options.y2axis);
		...
		for(i = neededColors - 1; i > -1; --i){
			...
			if(c){
				--neededColors;
				if($type(c) === 'number') assignedColors.push(c);
		...
		for(i = 0, j = 0; i < ln; ++i){
			...
			for (var t in Flotr.graphTypes){
				s[t] = $extend($extend({}, this.options[t]), s[t]);
			}
			s.mouse = $extend($extend({}, this.options.mouse), s.mouse);
	...
	constructCanvas: function(){
		...
		this.canvas = el.getFirst('.flotr-canvas');
		this.overlay = el.getFirst('.flotr-overlay');
		...
		c = this.canvas.setProperties(size).show().setStyles(style);
		...
		oc = this.overlay.setProperties(size).show().setStyles(style);
  ...
  processColor: function(color, options){
    ...
    if ($type(color) === 'string') return Flotr.Color.parse(color).scale(null, null, null, options.opacity).toString();
		
    var grad = color.colors ? color : {colors: color};
    
    if (!options.ctx) {
      if ($type(grad.colors) !== 'array') return 'rgba(0, 0, 0, 0)';
      return Flotr.Color.parse($type(grad.colors[0]) === 'array' ? grad.colors[0][1] : grad.colors[0]).scale(null, null, null, options.opacity).toString();
    }
    grad = $extend({start: 'top', end: 'bottom'}, grad); 
	...
	getTextDimensions: function(text, canvasStyle, HtmlStyle, className) {
		...
		if (!this.options.HtmlText && this.textEnabled) {
			...
			var dummyDiv = this.el.appendHTML('<div style="position:absolute;top:-10000px;'+HtmlStyle+'" class="'+className+' flotr-dummy-div">' + text + '</div>').getFirst(".flotr-dummy-div"),
    ...
    **/
		for(i = 0; i < s.length; ++i){
			s[i].data.each(function(v) {
				var x = v[0],
				    y = v[1], 
					r = dg.filter(function(row) {return row[0] == x})[0];
				if (r) r[i+1] = y;
				else {
					var newRow = [];
					newRow[0] = x;
    ...
    // The data grid is sorted by x value
		return this.seriesData = dg.sort(function(v){return v[0]});
	},
	/**
	...
	initEvents: function () {
		...
		this.overlay.removeEvents()
		    .addEvent('mousedown', this.mouseDownHandler.bindWithEvent(this))
		    .addEvent('mousemove', this.mouseMoveHandler.bindWithEvent(this))
		    .addEvent('click', this.clickHandler.bindWithEvent(this));
	},
	/**
	...
	draw: function() {
		...
		if(this.series.length){
			this.el.fireEvent('flotr:beforedraw', [this.series, this]);
		...
		this.drawOutline();
		this.el.fireEvent('flotr:afterdraw', [this.series, this]);
	},
	/**
	...
	drawGrid: function(){
		...
		if(o.grid.verticalLines || o.grid.horizontalLines){
			this.el.fireEvent('flotr:beforegrid', [this.axes.x, this.axes.y, o, this]);
		}
		ctx.save();
		ctx.lineWidth = 1;
		...
		ctx.restore();
		if(o.grid.verticalLines || o.grid.horizontalLines){
			this.el.fireEvent('flotr:aftergrid', [this.axes.x, this.axes.y, o, this]);
		}
	}, 
	/**
   ...
   */
	...
	insertLegend: function(){
		...
		var noLegendItems = series.filter(function(s) {return (s.label && !s.hide)}).length;
				...
				for(i = 0; i < series.length; i++){
		  		...
		  		if(fragments.length > 0){
		  			...
		  			if(options.legend.container != null){
		  				...
		  				this.el.appendHTML('<div class="flotr-legend" style="position:absolute;z-index:2;' + pos +'">' + table + '</div>');
		  				var div = this.el.getFirst('.flotr-legend');
		  				...
		  				if(options.legend.backgroundOpacity != 0.0){
		  					...
		  					this.el.appendHTML('<div class="flotr-legend-bg" style="position:absolute;width:' + div.getSize().x + 'px;height:' + div.getSize().y + 'px;' + pos +'background-color:' + c + ';"> </div>')
                    .getFirst('div.flotr-legend-bg').setStyle('opacity', options.legend.backgroundOpacity);
		  				}
		  			}
		  		}
		    }
	...
	getEventPosition: function (event){
		var offset = this.overlay.getCoordinates(),
		    pointer = event.page,
	...
	clickHandler: function(event){
		...
		this.el.fireEvent('flotr:click', [this.getEventPosition(event), this]);
	},
	/**
	...
	mouseMoveHandler: function(event){
		...
		if(this.selectionInterval == null && (this.options.mouse.track || this.series.some(function(s){return s.mouse && s.mouse.track;})))
			this.hit(pos);
		
		if (this.options.crosshair.mode)
			this.drawCrosshair(pos);
    
		this.el.fireEvent('flotr:mousemove', [event, pos, this]);
	},
	/**
	...
	fireSelectEvent: function(){
		...
		this.el.fireEvent('flotr:select', [{
			x1:Math.min(x1, x2), 
			y1:Math.min(y1, y2), 
			x2:Math.max(x1, x2), 
			y2:Math.max(y1, y2),
	...
	mouseUpHandler: function(event){
		$(document).removeEvent('mouseup', this.mouseUpHandler);
		event.stop();
	...
	setSelectionPos: function(pos, event) {
		var options = this.options,
		    offset = $(this.overlay).getCoordinates();
	...
	drawCrosshair: function(pos) {
		...
		if (pos.relX < 0 || pos.relY < 0 || pos.relX > this.plotWidth || pos.relY > this.plotHeight) {
			...
			this.el.removeClass('flotr-crosshair');
			return; 
		}
		...
		if (options.crosshair.hideCursor) {
			this.el.style.cursor = Browser.Engine.gecko ? 'none' :'url(blank.cur),crosshair';
			this.el.addClass('flotr-crosshair');
		}
	...
	hit: function(mouse){
		...
		if(n.series && (n.mouse && n.mouse.track && !prevHit || (prevHit /*&& (n.x != prevHit.x || n.y != prevHit.y)*/))){
			var mt = this.mouseTrack || this.el.getFirst(".flotr-mouse-value"),
			    pos = '', 
			    s = n.series,
			    p = n.mouse.position, 
			    m = n.mouse.margin,
			...
			if(!mt){
				this.el.appendHTML('<div class="flotr-mouse-value" style="'+elStyle+'"></div>');
	...
	saveImage: function (type, width, height, replaceCanvas) {
		...
		if ($type(image) === 'element' && replaceCanvas) {
			this.restoreCanvas();
			this.canvas.hide();
			this.overlay.hide();
			this.el.grab(image.setStyles({position: 'absolute'}));
		}
	},
	restoreCanvas: function() {
		this.canvas.show();
		this.overlay.show();
		this.el.getElements('img').dispose();
	}
});

Flotr.Color = new Class({
...
$extend(Flotr.Color, {
	...
	parse: function(color){
		...
		var name = (color+'').trim().toLowerCase();
		if(name == 'transparent'){
...
Flotr.addType('pie', {
	...
	draw: function(series) {
		...
		// Pie portions
		portions = this.series.map(function(hash, index){
		...
		sum = portions.pluck('value').pluck(1).sum(),
		fraction = 0.0,
		angle = series.pie.startAngle,
		value = 0.0;
		
		var slices = portions.map(function(slice){
			angle += fraction;
...
Flotr.addPlugin('spreadsheet', {
    ...
    'flotr:afterconstruct': function(){
      this.el.getElements('.flotr-tabs-group,.flotr-datagrid-container').dispose();
      
      if (!this.options.spreadsheet.show) return;
      
      var ss = this.spreadsheet;
      ss.tabsContainer = new Element('div', {style:'position:absolute;left:0px;width:'+this.canvasWidth+'px'}).addClass('flotr-tabs-group');
  		ss.tabs = {
  			graph: new Element('div', {style:'float:left'}).addClass('flotr-tab selected').set('html', this.options.spreadsheet.tabGraphLabel),
  			data: new Element('div', {style:'float:left'}).addClass('flotr-tab').set('html', this.options.spreadsheet.tabDataLabel)
  		};
  		ss.tabsContainer.grab(ss.tabs.graph).grab(ss.tabs.data);
      
      this.el.grab(ss.tabsContainer, 'bottom');
      
      var offset = ss.tabs.data.getSize().y + 2;
      this.plotOffset.bottom += offset;
      ss.tabsContainer.setStyles({top: this.canvasHeight-offset+'px'});
      
  		ss.tabs.graph.addEvent('click', function(){ss.showTab('graph')});
  		ss.tabs.data.addEvent('click', function(){ss.showTab('data')});
  	}
  },
  /**
		...
		if (!Browser.Engine.trident) {
			t.getElements('td').each(function(td) {
				td.addEvent('mouseover', function(e){
					td = $(e.target);
					var siblings = td.getAllPrevious();
					
					t.getElements('th[scope=col]')[siblings.length-1].addClass('hover');
					t.getElements('colgroup col')[siblings.length].addClass('hover');
				}).addEvent('mouseout', function(){
					t.getElements('colgroup col.hover, th.hover').removeClass('hover');
				});
			});
		}
	...
	showTab: function(tabName){
		...
		switch(tabName) {
			...
			case 'data':
				...
				this.spreadsheet.datagrid.getParent().show();
				this.el.getElements(selector).hide();
				this.spreadsheet.tabs.data.addClass('selected');
				this.spreadsheet.tabs.graph.removeClass('selected');
			break;
		}
	},
	/**
