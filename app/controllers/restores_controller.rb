class RestoresController < ApplicationController
  ...
  def create
    ...
    if file_indices && @restore.save!
      redirect_to host_restore_schedule_file_indices_path(params[:host_id], @restore.id, params[:schedule][:schedule_id])
    else
      flash[:error] = "Had a problem creating the restore. Either there are no snapshots for this host and schedule or there was a problem with the database!"
      @host = Host.find(params[:host_id])
      @restore = Restore.new
      @schedules = @host.schedules
      render :action => :new
